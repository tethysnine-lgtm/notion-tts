...
// 청크 1개 = ElevenLabs 호출 1회.
// 프론트엔드가 /api/tts 에서 받은 청크 배열을 이 엔드포인트로 순차 호출한다.
// 요청당 단일 합성만 수행하므로 Vercel 무료 플랜의 10초 타임아웃을 피한다.
//
...
async function synthesize(
  ...
  if (!res.ok) {
    ...
    console.error("[/api/tts/chunk] ElevenLabs 응답 오류:", {
      status: res.status,
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 단일 청크만 처리한다. 너무 길면 타임아웃 방지를 위해 거부.
    ...
    console.error("[/api/tts/chunk] 오류:", {
      name: err?.name,
      message: err?.message,
      status: err?.status,
      stack: err?.stack,
